<template>
  <div class="p-6">
    ...
    <div class="mb-6 flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Recursos</h1>
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
            ...
            <th class="px-4 py-3 text-right font-semibold text-gray-600 dark:text-gray-300">Costo Mensual</th>
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-700/50">
          ...
          <template v-if="loadingList">
            <tr v-for="n in 5" :key="n" class="bg-white dark:bg-gray-800">
          ...
          >
            ...
            <td class="px-4 py-3">
              <span :class="tipoBadgeClass(recurso.tipo)">{{ tipoLabel(recurso.tipo) }}</span>
            </td>
          ...
          <div v-if="showModal" class="relative z-10 w-full max-w-lg rounded-2xl bg-white dark:bg-gray-900 shadow-2xl">
            ...
            <div class="space-y-4 px-6 py-6">
              <div>
                ...
                <input
                  v-model="form.nombre"
                  type="text"
                  placeholder="Ej: Torno CNC, Operario de planta..."
              ...
              <div>
                ...
                <p v-if="getFieldError(apiState.validationErrors.value, 'tipo')" class="mt-1.5 text-xs text-red-500">
                  {{ getFieldError(apiState.validationErrors.value, 'tipo') }}
                </p>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Costo mensual ($)</label>
                <input
                  v-model.number="form.costo_mensual"
                  type="number"
                  min="0"
                  step="0.01"
                  placeholder="Ej: 3000"
                  :class="inputClass('costo_mensual')"
                />
                <p v-if="getFieldError(apiState.validationErrors.value, 'costo_mensual')" class="mt-1.5 text-xs text-red-500">
                  {{ getFieldError(apiState.validationErrors.value, 'costo_mensual') }}
                </p>
              </div>
            </div>
            <div class="flex items-center justify-end gap-3 border-t border-gray-100 dark:border-gray-800 px-6 py-4">
              ...
              >
                ...
                {{ apiState.loading.value ? 'Guardando...' : (editingId ? 'Guardar cambios' : 'Crear recurso') }}
              </button>
            </div>
          </div>
        </Transition>
